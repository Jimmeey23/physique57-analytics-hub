...
const SHEET_ID = process.env.NOTES_SHEET_ID || "";
...
async function ensureHeaderAndSheet(accessToken: string): Promise<void> {
  ...
  await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}:batchUpdate`, {
    ...
    body: JSON.stringify({
      requests: [{ addSheet: { properties: { title: NOTES_SHEET_NAME } } }],
    }),
  });

  await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(NOTES_SHEET_NAME)}!A1:I1?valueInputOption=RAW`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` },
      body: JSON.stringify({
        values: [["timestamp", "tableKey", "location", "period", "sectionId", "author", "note", "summary", "version"]],
      }),
    }
  );
}
...
async function getSheetId(accessToken: string): Promise<number> {
  const metaResp = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}?fields=sheets(properties(sheetId%2Ctitle))`,
    { headers: { Authorization: `Bearer ${accessToken}` } }
  );
...
router.get("/notes", async (req, res) => {
  ...
  try {
    ...
    req.log.warn({ err: readError }, "Notes API read unavailable");
  ...
  try {
    ...
    if (row && typeof row === "number") {
      ...
      const appendResp = await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(NOTES_SHEET_NAME)}!A:I:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` },
          body: JSON.stringify({ values }),
    ...
    req.log.error({ err }, "Notes API error");
...
router.delete("/notes", async (req, res) => {
  try {
    const accessToken = await getAccessToken();
    const rowParam = (req.query?.row ?? req.query?.rowNumber ?? req.body?.row ?? req.body?.rowNumber) as string;
    ...
    req.log.error({ err }, "Notes API delete error");
    return res.status(500).json({ error: "Internal server error" });
  }
});
